...
const AdminPanel = () => {
  ...
  const updatePaymentSetting = async (method: string) => {
    ...
    await handleAdminAction("update_payment_settings", {
      method,
      accountNumber: setting.number,
      accountName: setting.name,
      minDeposit: 50,
      minWithdrawal: 100,
    });
  };
        ...
        >
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Manage deposits, withdrawals, and settings
            </p>
          </div>
        </motion.div>
                ...
                >
                  <div className="flex-1">
                    <p className="font-medium">
                      {deposit.profiles?.username || deposit.profiles?.email || "Unknown"}
                    </p>
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      onClick={() =>
                        handleAdminAction("approve_deposit", { depositId: deposit.id })
                      }
                      disabled={actionLoading === `approve_deposit-${deposit.id}`}
                    >
                      {actionLoading === `approve_deposit-${deposit.id}` ? (
                    ...
                    <Button
                      ...
                      variant="destructive"
                      onClick={() =>
                        handleAdminAction("reject_deposit", {
                          depositId: deposit.id,
                          note: "Rejected by admin",
                        })
                      }
                      disabled={actionLoading === `reject_deposit-${deposit.id}`}
                    >
                      {actionLoading === `reject_deposit-${deposit.id}` ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
            ...
            {pendingWithdrawals.length === 0 ? (
              ...
              pendingWithdrawals.map((withdrawal) => (
                ...
                >
                  <div className="flex-1">
                    <p className="font-medium">
                      {withdrawal.profiles?.username || withdrawal.profiles?.email || "Unknown"}
                    </p>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      onClick={() =>
                        handleAdminAction("approve_withdrawal", {
                          withdrawalId: withdrawal.id,
                        })
                      }
                      disabled={actionLoading === `approve_withdrawal-${withdrawal.id}`}
                    >
                      {actionLoading === `approve_withdrawal-${withdrawal.id}` ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
                        <CheckCircle className="w-4 h-4 mr-1" />
                      )}
                    ...
                    <Button
                      ...
                      variant="destructive"
                      onClick={() =>
                        handleAdminAction("reject_withdrawal", {
                          withdrawalId: withdrawal.id,
                          note: "Rejected by admin",
                        })
                      }
                      disabled={actionLoading === `reject_withdrawal-${withdrawal.id}`}
                    >
                      {actionLoading === `reject_withdrawal-${withdrawal.id}` ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
                        <XCircle className="w-4 h-4 mr-1" />
                      )}
              ...
              <div key={method} className="glass rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Input
                    ...
                    value={paymentSettings[method]?.number || ""}
                    onChange={(e) =>
                      setPaymentSettings((prev) => ({
                        ...prev,
                        [method]: { ...prev[method], number: e.target.value },
                      }))
                    }
                  />
                  <Input
                    placeholder="Account Name (optional)"
                    value={paymentSettings[method]?.name || ""}
                    onChange={(e) =>
                      setPaymentSettings((prev) => ({
                        ...prev,
                        [method]: { ...prev[method], name: e.target.value },
                      }))
                    }
                  />
                </div>
                <Button size="sm" onClick={() => updatePaymentSetting(method)}>
                  Save {method}
                </Button>
              </div>
            ))}
